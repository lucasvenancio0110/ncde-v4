...
export default function App() {
  const [factory, setFactory] = useState<FactoryState>(initialState);
  ...
  function addPreparador() {
    const id = `preparador-${Date.now()}`;
    setResult(null);
    setFactory((current) => ({
      ...current,
      preparadores: [
        ...current.preparadores,
        { id, nome: '', status: 'livre', jantarConcluido: false },
      ],
    }));
  }
  ...
  function updateSetup(id: string, field: string, value: string) {
    ...
    setFactory((current) => ({
      ...
      setups: current.setups.map((setup) => {
        ...
        if (field === 'duracaoEstimadaMin') {
          return { ...setup, duracaoEstimadaMin: Number(value) };
        }
  ...
  function removeSetup(id: string) {
    setResult(null);
    setFactory((current) => ({ ...current, setups: current.setups.filter((setup) => setup.id !== id) }));
  }
    ...
    <main className="shell">
      <header className="hero">
        ...
        <span className={`status ${result?.valid ? 'ok' : result ? 'risk' : 'idle'}`}>
          {result ? (result.valid ? 'PLANO SEGURO' : 'AÇÃO NECESSÁRIA') : 'AGUARDANDO DADOS'}
        </span>
      </header>
      ...
      <section className="workspace">
            ...
            <div className="form-grid two-columns">
              <label>
                ...
                <input
                  ...
                  onChange={(event) => {
                    setResult(null);
                    setFactory((current) => ({ ...current, agora: event.target.value }));
                  }}
                />
              </label>
              <label>
                ...
                <input
                  type="time"
                  value={factory.fimTurno}
                  onChange={(event) => {
                    setResult(null);
                    setFactory((current) => ({ ...current, fimTurno: event.target.value }));
                  }}
                />
              </label>
            </div>
                ...
                <article className="editable-card" key={person.id}>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) => updatePreparador(person.id, 'nome', event.target.value)}
                    />
                  </label>
                  <div className="form-grid two-columns">
                    <label>
                      ...
                      <select
                        value={person.status}
                        onChange={(event) => updatePreparador(person.id, 'status', event.target.value)}
                      >
                        {statuses.map((status) => (
                    ...
                    <label>
                      ...
                      <input
                        type="time"
                        value={person.compromissoAte ?? ''}
                        onChange={(event) => updatePreparador(person.id, 'compromissoAte', event.target.value)}
                      />
                    </label>
                  </div>
                  <button className="danger-button" type="button" onClick={() => removePreparador(person.id)}>
                    Remover preparador
                  </button>
                </article>
              ))}
                ...
                <article className="editable-card" key={setup.id}>
                  <div className="form-grid two-columns">
                    <label>
                      ...
                      <input
                        value={setup.maquina}
                        placeholder="Ex.: TNL 130"
                        onChange={(event) => updateSetup(setup.id, 'maquina', event.target.value)}
                      />
                    </label>
                    <label>
                      Horário
                      <input
                        type="time"
                        value={setup.horario}
                        onChange={(event) => updateSetup(setup.id, 'horario', event.target.value)}
                      />
                    </label>
                    <label>
                      ...
                      <input
                        type="number"
                        min="1"
                        value={setup.duracaoEstimadaMin}
                        onChange={(event) => updateSetup(setup.id, 'duracaoEstimadaMin', event.target.value)}
                      />
                    </label>
                    <label>
                      Responsável
                      <select
                        value={setup.responsavelId ?? ''}
                        onChange={(event) => updateSetup(setup.id, 'responsavelId', event.target.value)}
                      >
                        <option value="">Motor decide</option>
                        {factory.preparadores.map((person) => (
                          <option key={person.id} value={person.id}>{person.nome || 'Sem nome'}</option>
                        ))}
                      </select>
                    </label>
                  </div>
                  <button className="danger-button" type="button" onClick={() => removeSetup(setup.id)}>
                    Remover setup
                  </button>
                </article>
              ))}
        ...
        <section className="panel result-panel">
          <div className="panel-heading">
            ...
            <strong>{factory.agora}</strong>
          </div>

          {!result && (
            <div className="result-placeholder">
              <strong>O motor ainda não foi executado.</strong>
              <p>Preencha o estado do turno e toque em “Gerar plano”.</p>
            </div>
          )}
                ...
                {result.actions.map((action, index) => (
                  <article className="action" key={`${action.preparadorId}-${action.tipo}-${index}`}>
                    ...
                    <div>
                      ...
                      {action.fim && <span className="action-end">Até {action.fim}</span>}
                    </div>
                  </article>
                ))}
              </div>
